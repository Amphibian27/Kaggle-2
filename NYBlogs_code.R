...
Train$NewsDesk<-ifelse(Train$NewsDesk=='',"Missing",Train$NewsDesk)
...
library(caTools)
sample=sample.split(Train$Popular,0.7)
Train1<-subset(Train,sample==T)
Test1<-subset(Train,sample==F)
Train2<-Train1[,c(1,2,3)]
Train.new <- dummy.data.frame(Train2, sep = ".")
Train_main<-cbind(Train.new,Train1[,-c(1,2,3)])
Train_main<-Train_main[,-c(40,41,42,44,46)]
M1=glm(Popular~.,data=Train_main,family=binomial)
  summary(M1)
PredM1 = predict(M1, newdata=Test1$Popular, type="response")
#AUC=0.56293
